...
**The emitted source is deliberately not marked as generated code.** Hint names end
`.ErrorCodes.cs`, not `.g.cs`, no member carries `[GeneratedCode]`, and there is no
`// <auto-generated/>` header. All three omissions are load-bearing in the same
direction: Roslyn suppresses analyzers on source it considers generated, and
coverlet's default `ExcludeByAttribute` would drop the members from the coverage
denominator with it, so `GeneratedErrorCodeTests` would stop counting.
`EmitsAHintNameThatIsNotTreatedAsGeneratedCode` holds the line.
