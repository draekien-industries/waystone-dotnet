...
code it never wrote. `[GeneratedCode]` is still absent: that specific attribute is
what coverlet's default `ExcludeByAttribute` keys on, and dropping the generated
members from the coverage denominator would make `GeneratedErrorCodeTests` stop
counting. `EmitsAHintNameThatIsTreatedAsGeneratedCode` holds the line.
