...
public static partial class OptionOfTAsyncExtensions
{
    ...
    /// passing the result of a function call, it is recommended to use OkOrElseAsync ,
    ...
    /// </returns>
    public static async Task<Result<TOk, TErr>> OkOrAsync<TOk, TErr>(
    ...
    /// A value task which when completed will return an
    /// option.
    /// </param>
    /// <param name="error">The error.</param>
    /// <returns>
    ...
    /// <see cref="Some{T}" />, otherwise an <see cref="Err{TOk, TErr}" />.
    /// </returns>
    public static async ValueTask<Result<TOk, TErr>> OkOrAsync<TOk, TErr>(
