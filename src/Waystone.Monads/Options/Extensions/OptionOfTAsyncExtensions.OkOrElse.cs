...
public static partial class OptionOfTAsyncExtensions
{
    ...
    /// </returns>
    public static Task<Result<TOk, TErr>> OkOrElseAsync<TOk, TErr>(this Option<TOk> option, Func<Task<TErr>> errorFactory)
        where TOk : notnull
        where TErr : notnull => option.MatchAsync(some => Task.FromResult(Result.Ok<TOk, TErr>(some)), async () =>
                                     {
                                         var error = await errorFactory().ConfigureAwait(false);
    ...
    /// An <see cref="Ok{TOk, TErr}"/> if the current option is a <see cref="Some{T}"/>, otherwise an <see cref="Err{TOk, TErr}"/>.
    /// </returns>
    public static async Task<Result<TOk, TErr>> OkOrElseAsync<TOk, TErr>(this Task<Option<TOk>> optionTask, Func<Task<TErr>> errorFactory)
          where TOk : notnull
          where TErr : notnull
    {
    ...
    /// An <see cref="Ok{TOk, TErr}"/> if the current option is a <see cref="Some{T}"/>, otherwise an <see cref="Err{TOk, TErr}"/>.
    /// </returns>
    public static ValueTask<Result<TOk, TErr>> OkOrElseAsync<TOk, TErr>(this Option<TOk> option, Func<ValueTask<TErr>> errorFactory)
        where TOk : notnull
        where TErr : notnull => option.MatchAsync(some => new ValueTask<Result<TOk, TErr>>(Result.Ok<TOk, TErr>(some)), async () =>
                                     {
                                         var error = await errorFactory().ConfigureAwait(false);
                                         return Result.Err<TOk, TErr>(error);
                                     });
    ...
    /// An <see cref="Ok{TOk, TErr}"/> if the current option is a <see cref="Some{T}"/>, otherwise an <see cref="Err{TOk, TErr}"/>.
    /// </returns>
    public static async ValueTask<Result<TOk, TErr>> OkOrElseAsync<TOk, TErr>(this ValueTask<Option<TOk>> optionTask, Func<ValueTask<TErr>> errorFactory)
          where TOk : notnull
          where TErr : notnull
    {
        var option = await optionTask.ConfigureAwait(false);
