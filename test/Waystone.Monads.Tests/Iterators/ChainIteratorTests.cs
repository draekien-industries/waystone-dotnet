...
public sealed class ChainIteratorTests
{
    [Fact]
    public void
        GivenTwoIteratorsThatHaveBeenChained_WhenTheFirstIteratorEnds_ThenIterateOverTheSecond()
    {
        var first = new[] { 1, 2, 3, 4 };
        var second = new[] { 5, 6, 7, 8 };

        ArrayIterator<int> firstIter = first.IntoIter();
        ArrayIterator<int> secondIter = second.IntoIter();

        ChainIterator<int> chained = firstIter.Chain(secondIter);
    ...
    public void GivenThreeIteratorsThatHaveBeenChanged_ThenIterateOverAllThree()
    {
        ...
        ChainIterator<int> chained = first.IntoIter()
                                          .Chain(second)
                                          .Chain(third);
