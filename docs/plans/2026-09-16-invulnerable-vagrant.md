...
no "already identified" failure to report and no `VAG-APR` code at all. Appraisal is the
context with no error family, which is the point of it: absence answers every question
it is asked.
...
caller has to remember to check. `LineItems.Of` reports `VAG-ORD-0001` at the one place
an empty set can be presented.
...
Catalog fails: 404 for an item that is not stocked, 409 for `VAG-CAT-0002` when too few
are on hand.
...
Codes are generated by `Waystone.Monads.SourceGenerators`, one family per context.

| Code | Meaning |
| --- | --- |
| `VAG-CAT-0001` | asking price is below the floor price |
| `VAG-CAT-0002` | not enough on hand to withdraw |
| `VAG-ORD-0001` | a purchase must carry at least one line item |
| `VAG-ORD-0002` | offer falls below what the shop will take |
| `VAG-ORD-0003` | tendered coin falls short of the total |
| `VAG-ORD-0004` | purchase has already settled |
| `VAG-ORD-0005` | buyback has already settled |
| `VAG-ORD-0006` | the till cannot cover the buyback |
| `VAG-STF-0001` | no clerk is free |
| `VAG-STF-0002` | clerk is already engaged |
...
| Status | Codes |
| --- | --- |
| 400 | a `SchemaViolation`, or `VAG-ORD-0001` |
| 402 | `VAG-ORD-0003`, `VAG-ORD-0006` |
| 404 | a `None` from a repository lookup — no code, because no reason is owed |
| 409 | `VAG-CAT-0002`, `VAG-ORD-0002`, `VAG-ORD-0004`, `VAG-ORD-0005`, `VAG-STF-0002` |
| 503 | `VAG-STF-0001` |

`VAG-CAT-0001` reaches no status code. `PriceBand.Between` is called when the shop stocks
an item, which happens in seeding, so the failure is a startup failure rather than a
response.
...
- **`Vagrant.Host/Endpoints`** — one file per resource, mapping `Result` to a status code.
  ...
  releases the clerk through `ReleaseAsync`. `VAG-STF-0001` becomes a 503 and the
...
- *Define Errors Out of Existence* — `Coin.FromGold` and `Coin.From` took `int`
  ...
  `Result`.
- *Fail Fast* — `void Identify` could not report `VAG-APR-0001`, which the taxonomy
...
- *Fail Fast* — `Buyback.Settle` returned `Result<Receipt, Error>` with no `VAG-ORD`
  code reachable from it. Added `VAG-ORD-0005`, buyback has already settled.
...
- *Principle of Least Astonishment* — `VAG-ORD-0005` had no status code to travel on,
...
- *Minimize Complexity* — `POST /orders` created and settled a purchase in one call, so
  `Purchase.Agree` had no moment to happen in and `VAG-ORD-0002` and `VAG-ORD-0004` were
...
- *Principle of Least Astonishment* — `POST /buybacks` listed a 400 and a 402 that
  nothing could produce. Split the same way, and `VAG-ORD-0006` added for a till that
